...
@RestController
@RequestMapping("/api/incidente-historiales")
@RequiredArgsConstructor
public class IncidenteHistorialController {

    private final IncidenteHistorialService service;
    ...
    @GetMapping("/{id}")
    public ResponseEntity<IncidenteHistorialDto.Response> findById(@PathVariable Integer id) {
        return service.findById(id).map(mapper::toResponse)
                .map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<IncidenteHistorialDto.Response> create(@Valid @RequestBody IncidenteHistorialDto.Request request) {
        IncidenteHistorial saved = service.save(mapper.toEntity(request));
        return ResponseEntity.status(HttpStatus.CREATED).body(mapper.toResponse(saved));
    }

    @PutMapping("/{id}")
    public ResponseEntity<IncidenteHistorialDto.Response> update(@PathVariable Integer id, @Valid @RequestBody IncidenteHistorialDto.Request request) {
