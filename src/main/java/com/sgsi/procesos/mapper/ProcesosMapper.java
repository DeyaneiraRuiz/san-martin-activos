...
public interface ProcesosMapper {
    ...
    @Mapping(source = "periodicidadId", target = "periodicidad.id")
    @Mapping(source = "nivelAutomatizacionId", target = "nivelAutomatizacion.id")
    void updateEntityFromRequest(ProcesoDto.Request request, @MappingTarget Proceso entity);

    void updateEntityFromRequest(ControlDto.Request request, @MappingTarget Control entity);

    void updateEntityFromRequest(DocumentoDto.Request request, @MappingTarget Documento entity);

    @Mapping(source = "activoId", target = "activo.id")
    @Mapping(source = "procesoId", target = "proceso.id")
    void updateEntityFromRequest(ActivoProcesoDto.Request request, @MappingTarget ActivoProceso entity);

    @Mapping(source = "procesoId", target = "proceso.id")
    @Mapping(source = "controlId", target = "control.id")
    void updateEntityFromRequest(ProcesoControlDto.Request request, @MappingTarget ProcesoControl entity);

    @Mapping(source = "procesoId", target = "proceso.id")
    @Mapping(source = "documentoId", target = "documento.id")
    void updateEntityFromRequest(ProcesoDocumentoDto.Request request, @MappingTarget ProcesoDocumento entity);
}
